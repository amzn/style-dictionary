...
function fileHeader(config) {
  var to_ret = '';
  // for backward compatibility we need to have the user explicitly hide them
  var hideDoNotEdit = (config.options && config.options.hideDoNotEdit);
  var hideTimestamp = (config.options && config.options.hideTimestamp);
  if (!hideDoNotEdit) {
    to_ret += '/**\n';
...
function iconsWithPrefix(prefix, properties, config) {
  return fileHeader(config) + _.chain(properties)
...
module.exports = {
  ...
  'css/variables': function(dictionary, config) {
    ...
    return fileHeader(config) + root_vars;
  },
   ...
   * ```
   */
  'scss/variables': function(dictionary, config) {
    return fileHeader(config) + variablesWithPrefix('$', dictionary.allProperties);
  },
   ...
   * ```
   */
  'less/variables': function(dictionary, config) {
    return fileHeader(config) + variablesWithPrefix('@', dictionary.allProperties);
  },
   ...
   * ```
   */
  'javascript/module': function(dictionary, config) {
    return fileHeader(config) +
      module_def +
   ...
   * ```
   */
  'javascript/umd': function(dictionary, config) {
   ...
   * ```
   */
  'javascript/es6': function(dictionary, config) {
    return fileHeader(config) +
