...
function variablesWithPrefix(prefix, properties) {
  return _.map(properties, ({ name, attributes, value, comment }) => {
    var to_ret_prop = `${prefix + name}: ${attributes.category === 'asset' ? `"${value}"` : value};`;
...
function iconsWithPrefix(prefix, properties, config) {
  return _.chain(properties)
    ...
    .map(({ name, value, attributes }) => {
      var varName = `${prefix + name}: ${value};`;
      var className = `.${config.prefix}-icon.${attributes.item}:before `;
      var declaration = `{ content: ${prefix}${name}; }`;
...
module.exports = {
  ...
  'javascript/es6': function({ allProperties }) {
    return (
      ...
      _.map(allProperties, ({ name, value, comment }) => {
        var to_ret_prop = `export const ${name} = ${JSON.stringify(value)};`;
        if (comment) to_ret_prop = to_ret_prop.concat(` // ${comment}`);
        return to_ret_prop;
      }).join('\n')
    );
   ...
   * ```
   */
  'json/asset': function({ properties }) {
    return JSON.stringify({ asset: properties.asset }, null, 2);
  },
