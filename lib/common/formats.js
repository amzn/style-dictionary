...
import dotnetClassCs from './templates/dotnet/class.cs.template.js';
...
const formats = {
  ...
  'scss/map-flat': async function ({ dictionary, options, file }) {
    const template = _template(scssMapFlat);
    const { allTokens } = dictionary;
    const header = await fileHeader({ file, commentStyle: 'long' });
    return template({ allTokens, file, options, header });
  },
   ...
   * @memberof Formats
   * @kind member
   * @typedef {Object} scssMapDeepOpts
   * @property {Boolean} [scssMapDeepOpts.outputReferences=false] - Whether or not to keep [references](/#/formats?id=references-in-output-files) (a -> b -> c) in the output.
   * @property {Boolean} [scssMapDeepOpts.themeable=true] - Whether or not tokens should default to being themeable, if not otherwise specified per token.
   * @param {FormatOpts & { options?: scssMapDeepOpts}} options
   * @example
   * ```scss
   ...
   * @memberof Formats
   * @kind member
   * @typedef {Object} scssVariablesOpts
   * @property {Boolean} [scssVariablesOpts.showFileHeader=true] - Whether or not to include a comment that has the build date
   * @property {Boolean} [scssVariablesOpts.outputReferences=false] - Whether or not to keep [references](/#/formats?id=references-in-output-files) (a -> b -> c) in the output.
   * @property {Boolean} [scssVariablesOpts.themeable=false] - Whether or not tokens should default to being themeable, if not otherwise specified per token.
   * @param {FormatOpts & { options?: scssVariablesOpts}} options
   * @example
   * ```scss
   * $color-background-base: #f0f0f0;
   * $color-background-alt: #eeeeee !default;
   ...
   * @memberof Formats
   * @kind member
   * @typedef {Object} lessVariablesOpts
   * @property {Boolean} [lessVariablesOpts.showFileHeader=true] - Whether or not to include a comment that has the build date
   * @property {Boolean} [lessVariablesOpts.outputReferences=false] - Whether or not to keep [references](/#/formats?id=references-in-output-files) (a -> b -> c) in the output.
   * @param {FormatOpts & { options?: lessVariablesOpts}} options
   * @example
   * ```less
   * \@color-background-base: #f0f0f0;
   * \@color-background-alt: #eeeeee;
   ...
   * ```
   */
  'javascript/module': async function ({ dictionary, file }) {
    const header = await fileHeader({ file });
   ...
   * ```
   */
  'javascript/object': async function ({ dictionary, file }) {
    const header = await fileHeader({ file });
    return (
      header +
      'var ' +
   ...
   * ```
   */
  'javascript/umd': async function ({ dictionary, file }) {
...
declare const ${moduleName}: ${JSON.stringify(treeWalker(dictionary.tokens), null, 2)}`;
   ...
   * @memberof Formats
   * @kind member
   * @typedef {Object} androidResourcesOpts
   * @property {Boolean} [androidResourcesOpts.showFileHeader=true] - Whether or not to include a comment that has the build date
   * @property {Boolean} [androidResourcesOpts.outputReferences=false] - Whether or not to keep [references](/#/formats?id=references-in-output-files) (a -> b -> c) in the output.
   * @param {FormatOpts & { options?: androidResourcesOpts }} options
   * @example
   * ```xml
  ...
  'android/colors': async function ({ dictionary, options, file }) {
    const template = _template(androidColors);
    const header = await fileHeader({ file, commentStyle: 'xml' });
    return template({ dictionary, file, options, header });
  },
  ...
  'android/dimens': async function ({ dictionary, options, file }) {
    const template = _template(androidDimens);
    const header = await fileHeader({ file, commentStyle: 'xml' });
    return template({ dictionary, file, options, header });
  },
  ...
  'android/fontDimens': async function ({ dictionary, options, file }) {
    const template = _template(androidFontDimens);
    const header = await fileHeader({ file, commentStyle: 'xml' });
    return template({ dictionary, file, options, header });
  },
  ...
  'android/integers': async function ({ dictionary, options, file }) {
    const template = _template(androidIntegers);
    const header = await fileHeader({ file, commentStyle: 'xml' });
    return template({ dictionary, file, options, header });
  },
  ...
  'android/strings': async function ({ dictionary, options, file }) {
    const template = _template(androidStrings);
    const header = await fileHeader({ file, commentStyle: 'xml' });
    return template({ dictionary, file, options, header });
  },
   ...
   * @property {Boolean} [composeObjectOpts.outputReferences=false] - Whether or not to keep [references](/#/formats?id=references-in-output-files) (a -> b -> c) in the output.
   * @param {FormatOpts & { options?: composeObjectOpts }} options
   * @example
   * ```kotlin
  ...
  'compose/object': async function ({ dictionary, options, file }) {
    ...
    const header = await fileHeader({ file, commentStyle: 'short' });
    return template({ allTokens: sortedTokens, file, options, formatProperty, header });
  },
  ...
  'ios/macros': async function ({ dictionary, options, file }) {
    ...
    const header = await fileHeader({ file, commentStyle: 'short' });
    return template({ dictionary, options, file, header });
  },
  ...
  'ios/singleton.m': async function ({ dictionary, options, file }) {
    const template = _template(iosSingletonM);
    const header = await fileHeader({ file, commentStyle: 'short' });
    return template({ dictionary, options, file, header });
  },
   ...
   * @todo Add example and usage
   */
  'ios/singleton.h': async function ({ dictionary, options, file }) {
    const template = _template(iosSingletonH);
    const header = await fileHeader({ file, commentStyle: 'short' });
    return template({ dictionary, options, file, header });
  },
   ...
   * @todo Add example and usage
   */
  'ios/static.h': async function ({ dictionary, options, file }) {
    const template = _template(iosStaticH);
    const header = await fileHeader({ file, commentStyle: 'short' });
    return template({ dictionary, options, file, header });
  },
  ...
  'ios/static.m': async function ({ dictionary, options, file }) {
    const template = _template(iosStaticM);
    const header = await fileHeader({ file, commentStyle: 'short' });
    return template({ dictionary, options, file, header });
  },
  ...
  'ios/colors.h': async function ({ dictionary, options, file }) {
    const template = _template(iosColorsH);
    const header = await fileHeader({ file, commentStyle: 'short' });
    return template({ dictionary, options, file, header });
  },
  ...
  'ios/colors.m': async function ({ dictionary, options, file }) {
    const template = _template(iosColorsM);
    const header = await fileHeader({ file, commentStyle: 'short' });
    return template({ dictionary, options, file, header });
  },
   ...
   * @todo Add example and usage
   */
  'ios/strings.h': async function ({ dictionary, options, file }) {
    const template = _template(iosStringsH);
    const header = await fileHeader({ file, commentStyle: 'short' });
    return template({ dictionary, options, file, header });
  },
  ...
  'ios/strings.m': async function ({ dictionary, options, file }) {
    const template = _template(iosStringsM);
    const header = await fileHeader({ file, commentStyle: 'short' });
    return template({ dictionary, options, file, header });
  },
   ...
   * @property {Boolean} [iosSwiftClassOpts.outputReferences=false] - Whether or not to keep [references](/#/formats?id=references-in-output-files) (a -> b -> c) in the output.
   * @param {FormatOpts & { options?: iosSwiftClassOpts }} options
   * @example
   * ```swift
   ...
   * @property {Boolean} [iosSwiftEnumOpts.outputReferences=false] - Whether or not to keep [references](/#/formats?id=references-in-output-files) (a -> b -> c) in the output.
   * @param {FormatOpts & { options?: iosSwiftEnumOpts }} options
   * @example
   * ```swift
   ...
   * @property {boolean} [iosSwiftAnyOpts.outputReferences=false] - Whether or not to keep [references](/#/formats?id=references-in-output-files) (a -> b -> c) in the output.
   * @param {FormatOpts & { options?: iosSwiftAnyOpts }} options
   * @example
   * ```swift
   * import UIKit
   ...
   * @property {Boolean} [flutterClassOpts.outputReferences=false] - Whether or not to keep [references](/#/formats?id=references-in-output-files) (a -> b -> c) in the output.
   * @param {FormatOpts & { options?: flutterClassOpts }} options
   * @example
   * ```dart
  ...
  /**
   * Creates a C# implementation file of a class with values. It uses a default of `public` access modifier.
   *
   * @memberof Formats
   * @kind member
   * @typedef {Object} CSharpClassOpts
   * @property {String} [CSharpClassOpts.accessModifier='public'] - [Access modifier](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/access-modifiers) of the generated C# object
   * @property {String} [CSharpClassOpts.className] - The name of the generated Swift class
   * @property {Boolean} [CSharpClassOpts.showFileHeader=true] - Whether or not to include a comment that has the build date
   * @property {Boolean} [CSharpClassOpts.outputReferences=false] - Whether or not to keep [references](/#/formats?id=references-in-output-files) (a -> b -> c) in the output.
   * @param {FormatOpts & { options?: CSharpClassOpts }} options
   * @example
   * ```csharp
   * public class StyleDictionary {
   *   public static Color ColorBackgroundDanger = Color(1.000f, 0.918f, 0.914f, 1f);
   * }
   * ```
   */
  'dotnet/class.cs': async function ({ dictionary, options, file }) {
    const { allTokens, tokens, unfilteredTokens } = dictionary;
    const template = _template(dotnetClassCs);
    const { outputReferences, usesDtcg } = options;
    options = setCSharpFileProperties(options);
    const formatProperty = createPropertyFormatter({
      outputReferences,
      dictionary,
      usesDtcg,
