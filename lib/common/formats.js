...
const { fileHeader, filterDictionary, formattedVariables, getTypeScriptType, iconsWithPrefix, minifyDictionary, sortByReference, createPropertyFormatter, sortByName } = require('./formatHelpers');
...
declare const ${moduleName}: ${JSON.stringify(treeWalker(dictionary.tokens), null, 2)}`;
   ...
   * @kind member
   */
   'json/original': function({dictionary}) {
    const getKeys = (obj, key) => {
      if (!Array.isArray(obj) || obj.length < 1) return [];
      const found = obj.find(el => el.hasOwnProperty(key))
      if (found == null) return [];
      return Object.keys(found[key]);
    }
    
    const originalKeys = getKeys(dictionary.allTokens, 'original');
    const filteredJSON = { color: filterDictionary(dictionary.tokens.color, 'original', ...originalKeys) };
