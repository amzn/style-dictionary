...
import { stripMeta } from '../utils/stripMeta.js';
...
const formats = {
  ...
  'javascript/esm': async function ({ dictionary, file, options }) {
    ...
    const dictionaryString = JSON.stringify(
      minify ? minifyDictionary(dictionary.tokens, options.usesDtcg) : dictionary.tokens,
      null,
      2,
    );

    return `${header}export default ${dictionaryString};\n`;
  },
...
declare const ${moduleName}: ${JSON.stringify(treeWalker(dictionary.tokens), null, 2)}`;
  ...
  json: function ({ dictionary, options }) {
    ...
    /** @type {boolean | { keep?: string[]; strip?: string[] }} */
    const stripMetaOptions = options.stripMeta;
    const sdMetaProps = ['attributes', 'filePath', 'name', 'path', 'comment'];
    let { tokens } = dictionary;

    if (stripMetaOptions) {
      /** @type {{ strip?: string[]; keep?: string[]; usesDtcg?: boolean }} */
      let opts = { usesDtcg };

      if (stripMetaOptions === true) {
