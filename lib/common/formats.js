...
declare const ${moduleName}: ${JSON.stringify(treeWalker(dictionary.tokens), null, 2)}`;
  ...
  /**
   * Creates a Swift implementation file of a class with values. It adds default `class` type, `public` access control and `UIKit` import.
   *
   * @memberof Formats
   * @kind member
   * @param {Object} options
  ...
  'ios-swift/class.swift': function({dictionary, options, file, platform}) {
    ...
    let allTokens;
    const { outputReferences } = options;
    file = setSwiftFileProperties(file, 'class', platform.transformGroup);
    const formatProperty = createPropertyFormatter({
      outputReferences,
      dictionary,
      formatting: {
  ...
  /**
   * Creates a Swift implementation file of an enum with values. It adds default `enum` type, `public` access control and `UIKit` import.
   *
   * @memberof Formats
   * @kind member
   * @example
  ...
  'ios-swift/enum.swift': function({dictionary, options, file, platform}) {
    ...
    let allTokens;
    const { outputReferences } = options;
    file = setSwiftFileProperties(file, 'enum', platform.transformGroup);
    const formatProperty = createPropertyFormatter({
      outputReferences,
      dictionary,
      formatting: {
  ...
  /**
   * Creates a Swift implementation file of any given type with values. It has by default `class` type, `public` access control and `UIKit` import.
   * 
   * ```javascript
   ...
   */
   'ios-swift/any.swift': function({ dictionary, options, file, platform }) {
    ...
    let allTokens;
    const { outputReferences } = options;
    file = setSwiftFileProperties(file, file.objectType, platform.transformGroup);
    const formatProperty = createPropertyFormatter({
      outputReferences,
      dictionary,
      formatting: {
