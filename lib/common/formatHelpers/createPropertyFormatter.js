...
/**
 * Creates a function that can be used to format a tokenerty. This can be useful
...
export default function createPropertyFormatter({
  ...
  return function (token) {
    ...
    to_ret_token += token?.attributes?.category === 'asset' ? `"${value}"` : value;
    ...
    if (token.comment && commentStyle !== 'none') {
      to_ret_token = addComment(to_ret_token, token.comment, mergedOptions);
    }
