 ...
 * @param {Array} exclude - The keys that need to be excluded from the filtered object
 ...
 * ```
 */
const filterDictionary = (obj, exclude, ...target) => {
  ...
  return Object.fromEntries(
    Object
      .entries(obj)
      .map(([k, v]) => target.includes(k) ? [k, v] : typeof v === 'object' ? [k, filterDictionary(v, exclude, target)] : [k, {}])
      .filter(([k, v]) => v == null ? true : false || _.isEmpty(v) || exclude.includes(k) ? false : true)
  )
}
