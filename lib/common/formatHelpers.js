  ...
  function sorter(a, b) {
    ...
    if (dictionary.usesReference(a.original.value)) {
      ...
      if (dictionary.usesReference(b.original.value)) {
        const aRef = dictionary.getReference(a.original.value)[0];
        const bRef = dictionary.getReference(b.original.value)[0];
  ...
  return allProperties
    .map(function(prop) {
      ...
      if (outputReferences && dictionary.usesReference(prop.original.value)) {
        var refs = dictionary.getReference(prop.original.value);

        value = prop.original.value.replace(regex, function(match, variable, offset, string) {
          const ref = refs
