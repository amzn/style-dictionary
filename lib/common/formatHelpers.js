 ...
 * @example
 * ```javascript
 * dictionary.allProperties.sort(sortAllProperties(dictionary))
 * ```
 * @param {Dictionary} dictionary
 * @returns function
 */
function sortAllProperties(dictionary) {
  return function(a, b) {
    const aComesFirst = -1;
    const bComesFirst = 1;

    // If a uses a reference and b doesn't, b might come before a
    // read on..
    if (dictionary.usesReference(a.original.value)) {
      if (dictionary.usesReference(b.original.value)) {
        const aRef = dictionary.getReference(a.original.value);
        const bRef = dictionary.getReference(b.original.value);
        ...
        // if a's reference has a reference it should come after b
        if (dictionary.usesReference(aRef.original.value)) {
          return bComesFirst
        } else {
          return aComesFirst;
        }
...
function formattedVariables(format, dictionary, outputReferences = false) {
  ...
  if (outputReferences) {
    allProperties = allProperties.sort(sortAllProperties(dictionary));
  }
