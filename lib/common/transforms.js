...
function processFontFamily(fontFamily) {
  ...
  if (typeof f === 'string' && f.includes(',')) {
    f = f.split(',').map((part) => part.trim());
  }

  if (Array.isArray(f)) {
    return f.map((part) => quoteWrapWhitespacedFont(part)).join(', ');
  }

  return quoteWrapWhitespacedFont(f);
}

/**
 * @param {number[]|string} cubicBezier
 */
function transformCubicBezierCSS(cubicBezier) {
...
export default {
  ...
  'fontFamily/css': {
    type: 'value',
    filter: (token, options) => (options.usesDtcg ? token.$type : token.type) === 'fontFamily',
    transform: (token, _, options) => {
      /** @type {string|string[]} */
      const fontFamily = options.usesDtcg ? token.$value : token.value;

      return processFontFamily(fontFamily);
    },
  },
  ...
  'cubicBezier/css': {
    type: 'value',
    filter: (token, options) => (options.usesDtcg ? token.$type : token.type) === 'cubicBezier',
    transform: (token, _, options) => {
      /** @type {number[]|string} */
      const cubicBezier = options.usesDtcg ? token.$value : token.value;
      return transformCubicBezierCSS(cubicBezier);
    },
  },
  ...
  'typography/css/shorthand': {
    ...
    transform: (token, platform, options) => {
      ...
      fontFamily = processFontFamily(fontFamily ?? 'sans-serif');
  ...
  'transition/css/shorthand': {
    ...
    transform: (token, _, options) => {
      ...
      return `${duration} ${transformCubicBezierCSS(timingFunction)} ${delay}`;
    },
  },
