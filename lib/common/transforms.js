...
export default {
  ...
  'color/dotnet/MAUIColor': {
    type: 'value',
    matcher: isColor,
    transformer: function (token, _, options) {
      const color = Color(options.usesDtcg ? token.$value : token.value).toRgb();
      const rFixed = (color.r / 255.0).toFixed(3);
      const gFixed = (color.g / 255.0).toFixed(3);
      const bFixed = (color.b / 255.0).toFixed(3);
      const aFixed = color.a.toFixed(3);
      return `new Color(red: ${rFixed}f, green: ${gFixed}f, blue: ${bFixed}f, alpha: ${aFixed}f)`;
    },
  },
};
