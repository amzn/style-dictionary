...
export default {
  ...
  'attribute/color': {
    ...
    matcher: isColor,
    transformer: function (token) {
      var color = Color(token.$value ?? token.value);
      return {
        hex: color.toHex(),
        rgb: color.toRgb(),
        hsl: color.toHsl(),
  ...
  'color/hex8android': {
    ...
    matcher: isColor,
    transformer: function (token) {
      var str = Color(token.$value ?? token.value).toHex8();
  ...
  'color/UIColor': {
    ...
    matcher: isColor,
    transformer: function (token) {
      var rgb = Color(token.$value ?? token.value).toRgb();
      return (
  ...
  'size/object': {
    ...
    transformer: function (token, options) {
      ...
      const val = parseFloat(_v);
      if (isNaN(val)) throwSizeError(token.name, _v, 'object');
  ...
  'size/remToSp': {
    ...
    transformer: function (token, options) {
      ...
      if (isNaN(val)) throwSizeError(token.name, _v, 'sp');
  ...
  'size/remToDp': {
    ...
    transformer: function (token, options) {
      ...
      const val = parseFloat(_v);
      const baseFont = getBasePxFontSize(options);
      if (isNaN(val)) throwSizeError(token.name, _v, 'dp');
  ...
  'size/px': {
    ...
    transformer: function (token) {
      const _v = token.$value ?? token.value;
      const val = parseFloat(_v);
      if (isNaN(val)) throwSizeError(token.name, _v, 'px');
      return val + 'px';
    },
  },
  ...
  'size/rem': {
    ...
    transformer: function (token) {
      const val = parseFloat(token.$value ?? token.value);
      if (isNaN(val)) throwSizeError(token.name, token.$value ?? token.value, 'rem');
      return val + 'rem';
    },
  },
  ...
  'size/remToPt': {
    ...
    transformer: function (token, options) {
      ...
      const val = parseFloat(_v);
      const baseFont = getBasePxFontSize(options);
      if (isNaN(val)) throwSizeError(token.name, _v, 'pt');
  ...
  'size/compose/em': {
    ...
    transformer: function (token) {
      const _v = token.$value ?? token.value;
      const val = parseFloat(_v);
      if (isNaN(val)) throwSizeError(token.name, _v, 'em');
      return val + '.em';
    },
  },
  ...
  'size/swift/remToCGFloat': {
    ...
    transformer: function (token, options) {
      ...
      const val = parseFloat(_v);
      const baseFont = getBasePxFontSize(options);
      if (isNaN(val)) throwSizeError(token.name, _v, 'CGFloat');
  ...
  'size/pxToRem': {
    ...
    transformer: (token, options) => {
      const baseFont = getBasePxFontSize(options);
      const _v = token.$value ?? token.value;
      const floatVal = parseFloat(_v);

      if (isNaN(floatVal)) {
        throwSizeError(token.name, _v, 'rem');
      }
  ...
  'color/hex8flutter': {
    ...
    matcher: isColor,
    transformer: function (token) {
      var str = Color(token.$value ?? token.value)
        .toHex8()
        .toUpperCase();
      return `Color(0x${str.slice(6)}${str.slice(0, 6)})`;
    },
