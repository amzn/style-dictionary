...
function propertySetup(property, name, path) {
  ...
  if (!property.original) {
    ...
    // Keep the original object properties by which we can key off them in the transforms
    to_ret.original = _.clone(property);
    // Copy the name by which we can have a base name to transform
    ...
    // An array of the path down the object tree by which we can use it to build readable names
