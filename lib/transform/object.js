...
function transformObject(obj, options, transformationContext = {}, path, transformedObj) {
  ...
  path = path || [];
  let {transformedPropRefs = [], deferredPropValueTransforms = []} = transformationContext;
  ...
  for (const name in obj) {
    ...
    const isPlainObject = _es6.isPlainObject(objProp);
    ...
    if (isPlainObject && 'value' in objProp) {
      ...
      deferredPropValueTransforms = deferredPropValueTransforms.filter((el) => el!=pathName);
