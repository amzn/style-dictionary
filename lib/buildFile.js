...
export default async function buildFile(file, platform = {}, dictionary, options) {
  ...
  if (
    ...
    if (platform.log?.warnings === 'error') {
      ...
      consoleLog(chalk.rgb(255, 140, 0)(warnNoFile));
    }
    return null;
  }
  ...
  if ((nested || tokenNamesCollisionCount === 0) && filteredReferencesCount === 0) {
    // eslint-disable-next-line no-console
    consoleLog(chalk.bold.green(`✔︎ ${fullDestination}`));
  } else {
    ...
    if (tokenNamesCollisionCount > 0) {
      ...
      } else if (platform?.log?.verbosity !== 'silent') {
        // eslint-disable-next-line no-console
        consoleLog(chalk.rgb(255, 140, 0).bold(warn));
      }
    }
