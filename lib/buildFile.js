...
function buildFile(destination, format, platform, dictionary, filter) {
  ...
  Object.keys(nameCollisionObj).forEach((propertyName) => {
    if(nameCollisionObj[propertyName].length > 1) {
      let collisions = nameCollisionObj[propertyName].map((properties) => {
        let propertyPathText = chalk.keyword(HIGHLIGHT_COLOR)(properties.path.join('.'));
        let valueText = chalk.keyword(WARNING_COLOR)(properties.value);
  ...
  if(propertyNamesCollisionCount > 0) {
    let propertyNamesCollisionWarnings = GroupMessages.flush(PROPERTY_NAME_COLLISION_WARNINGS).join('\n    ');
    let title = `While building ${chalk.keyword(HIGHLIGHT_COLOR).bold(destination)}, token collisions were found; output may be unexpected.`;
