...
function extend(opts) {
  ...
  if (options.source) {
    ...
    var props = combineJSON(options.source, true, function Collision(prop) {
      var str = `Collision detected at: ${prop.path.join('.')}! Original value: ${prop.target[prop.key]}, New value: ${prop.copy[prop.key]}`;
      handleErrors.add(ERROR_TYPE, str);
    });

    if(handleErrors.count(ERROR_TYPE)) {
      handleErrors.show(ERROR_TYPE, options.log === 'error' ? chalk.bold.red : chalk.bold.orange);
