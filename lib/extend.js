...
var combineJSON = require('./utils/combineJSON'),
    ...
    ErrorGroups = require('./utils/errorGroups');
...
function extend(opts) {
  ...
  if (options.source) {
    ...
    var ERROR_GROUP = ErrorGroups.PropertyValueCollisions;
    var props = combineJSON(options.source, true, function Collision(prop) {
      var err = new CustomError(
        ...
        {group: ErrorGroups.PropertyValueCollisions}
      );
      ErrorHandler.add(err);
    });

    if(ErrorHandler.count(ERROR_GROUP) > 0) {
      if (options.log === 'error') {
        ErrorHandler.flush(ERROR_GROUP, chalk.bold.red);
        throw new Error('Collisions detected');
      }
      else {
        ErrorHandler.flush(ERROR_GROUP, chalk.bold.orange);
      }
    }
