...
var combineJSON = require('./utils/combineJSON'),
    ...
    GroupErrors = require('./utils/groupErrors');

var PROPERTY_VALUE_COLLISIONS = GroupErrors.GROUP.PropertyValueCollisions;
  ...
  if (options.source) {
    ...
    var props = combineJSON(options.source, true, function Collision(prop) {
      var err = new CustomError(
        ...
        {group: PROPERTY_VALUE_COLLISIONS}
      );
      GroupErrors.add(err);
    });

    if(GroupErrors.count(PROPERTY_VALUE_COLLISIONS) > 0) {
      if (options.log === 'error') {
        GroupErrors.flush(PROPERTY_VALUE_COLLISIONS, chalk.bold.red);
        throw new Error('Collisions detected');
      }
      else {
        GroupErrors.flush(PROPERTY_VALUE_COLLISIONS, chalk.bold.orange);
      }
    }
