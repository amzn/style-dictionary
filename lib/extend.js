...
const ConfigLogger = require('./utils/logger/config');
...
function extend(opts) {
  ...
  Logger.config = new ConfigLogger();
  ...
  if (options.include) {
    ...
    to_ret.properties = combineJSON(options.include, true, null, false, to_ret.parsers);
  ...
  if (options.source) {
    ...
    var props = combineJSON(options.source, true, function(props) {
      Logger.config.addPropertyValueCollision(props);
    }, true, to_ret.parsers);
