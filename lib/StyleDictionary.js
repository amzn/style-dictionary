...
import { resolveRefs } from './utils/references/resolveRefsNew.js';
...
import { ReferenceError } from './utils/references/referenceError.js';
...
/**
 * @template {import('../types/DesignToken.d.ts').DesignToken} [T=Token]
 ...
 * @typedef {import('../types/ReferenceError.js').ReferenceErrorObject} RefErrorObj
 */
...
export default class StyleDictionary extends Register {
  ...
  async _exportPlatform(platform) {
    ...
    /** @type {TokenMap | TokenMap<TransformedToken>} */
    ...
    /**
     * @type {Map<string, RefErrorObj>}
     */
    let refErrors = new Map();
    ...
    while (!finished) {
      ...
      const { errors } = resolveRefs(/** @type {TokenMap<TransformedToken>} */ (tokenMap), {
      ...
      errors.forEach((err) => {
        const key = `${err.ref}-${err.type}`;
        if (!refErrors.has(key)) {
          refErrors.set(key, err);
        }
      });
      ...
      if (newDeferredPropCount === 0) {
        ...
        // the resolveRefs method will find the circular references
        // we do this in case there are multiple circular references
        const { errors: nestedErrors } = resolveRefs(
          /** @type {TokenMap<TransformedToken>} */ (tokenMap),
          {
            usesDtcg,
            mutateMap: true,
          },
    ...
    const refWarningCount = refErrors.size;
    if (refWarningCount > 0) {
      const refWarnings = [...refErrors.values()];
      const warningStrings = refWarnings
        .map((err) => {
          const tok = /** @type {TransformedToken} */ (err.token);
          if (err.type === 'not-found') {
            return `${tok.key}${tok.filePath ? `in ${tok.filePath}` : ''} tries to reference ${err.ref}, which is not defined.`;
          }
          const chain = /** @type {string[]} */ (err.chain);
          return `Circular definition cycle for ${tok.key}${tok.filePath ? `in ${tok.filePath}` : ''} => ${chain.join(', ')}`;
        })
        .join('\n');
      ...
      if (this.log.errors?.brokenReferences === throwBrokenReference) {
        throw new ReferenceError(err, [...refErrors.values()]);
    ...
    return {
      tokenMap: /** @type {TokenMap<TransformedToken>} */ (tokenMap),
