 ...
 * @typedef {import('../types/ReferenceError.js').ReferenceError} RefError
 */
      ...
      const warningStrings = refWarnings
        .map((err) => {
          ...
          if (err.type === 'not-found') {
            return `${tok.key}${tok.filePath ? `from ${tok.filePath}` : ''} tries to reference ${err.ref}, which is not defined.`;
          }
          const chain = /** @type {string[]} */ (err.chain);
          return `Circular definition cycle for ${tok.key} => ${chain.join(', ')}`;
        })
        .join('\n');
