 ...
 * @typedef {'default' | 'verbose' | 'silent'} Verbosity
 */
...
export default class StyleDictionary extends Register {
   ...
   * @returns
   */
  async init(verbosity) {
    return this.extend(undefined, true, verbosity);
  }
  ...
  async extend(config = this.config, mutateOriginal = false, verbosity, volume) {
    ...
    this.log = {
      ...
      ...options.log,
      // verbosity is a bit more complex
      // can be passed imperatively by constructor (e.g. when using CLI --verbose / --silent)
      // otherwise verbosity in user config or fallback to default
      verbosity: verbosity ?? options.log?.verbosity ?? this.log.verbosity,
    };
    this.options = {
      ...options,
      log: this.log,
