...
export default class StyleDictionary {
  ...
  static _transformGroup = transformGroup;
  static _format = format;
  static _action = action;
  static _filter = filter;
  static _fileHeader = {};
  static _parsers = []; // we need to initialise the array, since we don't have built-in parsers
    ...
    ['transform', 'transformGroup', 'format', 'action', 'filter', 'fileHeader', 'parser'].forEach(
      (prop) => {
        ...
        Object.defineProperty(this, _prop, {
          get: function () {
            if (prop === 'parser') {
              return [...this.constructor[`_${_prop}`], ...this[`__${_prop}`]];
            }
            return { ...this.constructor[`_${_prop}`], ...this[`__${_prop}`] };
          },
          set: function (v) {
