...
function filterProperties(dictionary, filter) {
  if (!filter) {
    ...
    if(_.isString(filter)) {
      console.log(JSON.stringify(dictionary.allProperties, null, 2));
      // filterFunc = function() { return true; };
      // filterFunc = _.matches({ "attributes": { "category": "color" }});
      filterFunc = function(o) { return o.comment === "this is a comment"; }

    } else if (_.isFunction(filter)) {
      filterFunc = filter;
    } else { // TODO: should we check "isObject()" here, and have a fallback else that throws an error?
