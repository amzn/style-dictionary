 ...
 */
import { resolveRefs } from './references/resolveRefsNew.js';
// import { resolveMap } from './references/resolveMap.js';
...
export function expandTokens(tokenMap, opts, platform) {
  ...
  // Resolve map prior to running shouldExpand check, because
  // sometimes whether we should expand depends on the resolved value.
  // We don't do anything with the errors at this stage.
  resolveRefs(/** @type {TokenMap<TransformedToken>} */ (copyResolved), {
    usesDtcg: uses$,
    objectsOnly: true,
    mutateMap: true,
  });
