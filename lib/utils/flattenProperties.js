...
function flattenProperties(properties, to_ret) {
  ...
  for(var name in properties) {
    if (properties.hasOwnProperty(name)) {
      ...
      // TODO: this is blowing the call stack when a circular reference exists in the value of a key/value pair with a key that is not 'value'
      //        e.g. foo { bar: { inherits: '{foo}' }}
