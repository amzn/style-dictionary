 ...
 *
 * When not passing value, you can turn on mutateMap to mutate
...
export function resolveRefs(_tokenMap, { trackRefs = true, ...opts } = {}) {
  ...
  if (options?.mutateMap && options.value) {
    throw new Error('`mutateMap` and `value` option cannot be combined.');
  }
  ...
  if (options?.value) {
    ...
    const { resolved, errors, refs } = resolveRefValueWrapper(
      /** @type {Partial<T>} */ ({ [valueKey]: options.value }),
      tokenMap,
      options,
    );
...
export function resolveMap(tokenMap, _opts = {}) {
  ...
  for (const [key, token] of Array.from(tokenMap)) {
    ...
    if (opts.trackRefs) {
      // there may be refs already due to transitive transforms, in that case combine them
      if (resolved.refs && resolved.refs.size > 0) {
        if (refs.size > 0) {
          // we can leave it as is if we have no refs to add
          resolved.refs = resolved.refs.union(refs);
        }
      } else {
        resolved.refs = refs;
      }
 ...
 * @param {Set<string>} [refs] - keeping track of refs
 */
export function resolveRefValueWrapper(
  ...
  for (const key in slice) {
    ...
    if (typeof prop === 'object') {
      ...
      const intermediateToPleaseTypescript = /** @type {Set<string>} */ (_refs);
      refs = refs.union(intermediateToPleaseTypescript);
  ...
  return { resolved: slice, errors, refs };
}
...
export function _resolveReferences(
  ...
  /** @type {{ resolved: unknown; errors: RefErrors; refs: Set<string>; }} */
  let to_ret = { resolved: value, errors: [], refs: new Set() };
    ...
    if (typeof refVal !== 'undefined') {
      if (typeof refVal === 'string' && usesReferences(refVal)) {
        ...
        if (Object.hasOwn(foundCirc, refVal)) {
          ...
          to_ret.refs = to_ret.refs.union(nestedRefs);
