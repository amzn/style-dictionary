...
export function resolveRefValueWrapper(
  ...
  if (!originalToken) {
    // create a clone for reference later on in error reporting
    // TODO: this may be expensive action to clone every token..
    originalToken = structuredClone(slice);
  }
 ...
 * @returns {{ resolved: unknown; errors: ReferenceErrors }}
 */
export function _resolveReferences(
  ...
  /** @type {{ resolved: unknown; errors: ReferenceErrors }} */
