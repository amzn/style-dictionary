  ...
  value.replace(regex, function(match, variable) {
    ...
    if (ref) {
      if (typeof ref === 'string') {
        console.log('replace', value, match, ref);
        ...
        // if evaluated value is not a string, we need to set to an object

        // we want to output the same object
        var refCopy = _.cloneDeep(ref);
        var stackCopy = stack.slice(0);

        // generate the correct path replacement
        var oldPath = stackCopy.pop().split('.');
        var newPath = stackCopy.shift().split('.');

        // iterate through refCopy leaf nodes and create correct name, path, and CTI
        var replaceValueNodes = function (obj) {
            if(typeof obj === 'object' && obj !== null) {
                if(obj.value) {
                    // @TODO @BUG: this work should be done by the formatting function... which we dont have accecss to here
                    obj.name = obj.name.replace(oldPath.join('_'), newPath.join('_'));

                    // set the path and attributes
                    obj.path = obj.path.join('.').replace(oldPath.join('.'), newPath.join('.')).split('.');
                    obj.attributes.category = newPath[0];
                    obj.attributes.type = newPath[1];
                    obj.attributes.item = newPath[2];
                    obj.attributes.subitem = newPath[3];
                    obj.attributes.state = newPath[4];
                }
                else {
                    Object.keys(obj).forEach(function(objKey) {
                        replaceValueNodes(obj[objKey]);
                    });
                }
            }
        };
        replaceValueNodes(refCopy);
