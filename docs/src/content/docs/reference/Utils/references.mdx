...
When using `value` option to resolve only for a single value, this property will be on the return object.
...
const { resolved, refs, errors } = resolveRefs(sd.tokenMap, { value: sd.tokens.border.value }); // "solid 2px #000"
...
// Not passing a value option means resolving the entire Map.
...
| opts.value       | `unknown`                                                                 | You can pass a `value` to resolve references for a single token value instead of an entire `tokenMap`. This option is mutually exclusive with `mutateMap` option.                                                                                                    |
