...
Besides the resolved map/value, it also will add a `refs` property to each tokens in the resolved map.
This keeps track of all references (including deep/nested) of a token value.
...
> When using `resolveRefs`, a `refs` property is added to each token in the Map, containing an array of reference strings.
> You can then use these keys on the Map if you also need the token data.
