...
This method is used internally by `formatPlatform`/`buildPlatform`/`exportPlatform` etc.
...
You can try-catch this method (or any method that's using this one internally) to catch for example broken references error, and debug its `errors` property.

```javascript title="build-tokens.js"
import { ReferenceError } from 'style-dictionary/utils';
import StyleDictionary from 'style-dictionary';
...
const sd = new StyleDictionary({
  tokens: {
    ...
    border: {
      // contains a not-found and a circular (self-)reference error
...
try {
  ...
  if (err instanceof ReferenceError) {
    console.log(err.errors); // ReferenceErrorObject(2)
  }
}
```

More info: [reference errors](/reference/utils/references#error-handling).
