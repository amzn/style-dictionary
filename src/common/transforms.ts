...
import type { TransformedToken, Config, PlatformConfig, Transform } from '../types';
 ...
 * @returns {boolean}
 */
export function isColor(token: TransformedToken, options: Config) {
  return (
 ...
 * @returns {boolean}
 */
function isAsset(token: TransformedToken, options: Config) {
 ...
 * @returns {boolean}
 */
function isContent(token: TransformedToken, options: Config) {
 ...
 * @returns {string}
 */
function wrapValueWith(character: string, token: TransformedToken, options: Config) {
 ...
 * @returns {string}
 */
function wrapValueWithDoubleQuote(token: TransformedToken, options: Config) {
 ...
 * @returns {string}
 */
function throwSizeError(name: string, value: string | number, unitType: string) {
 ...
 * @param {Config} options
 */
function processFontFamily(token: TransformedToken, options: Config) {
   ...
   * @returns
   */
  const processFamily = (_family: string | string[]) => {
    let family = _family;
 ...
 * @param {Config} options
 */
function transformCubicBezierCSS(token: TransformedToken, options: Config) {
  const val = options.usesDtcg ? token.$value : token.value;
  const type = options.usesDtcg ? token.$type : token.type;

  /** @param {number[]|string} easing */
  const transformEasing = (easing: number[] | string) => {
 ...
 */
const transforms: Record<string, Omit<Transform, 'name'>> = {
  /**
  ...
  'attribute/cti': {
    ...
    transform: function (token) {
      ...
      const generatedAttrs: Record<string, string> = {};
  ...
  'html/icon': {
    ...
    transform: function (token, _, options) {
      return (options.usesDtcg ? token.$value : token.value).replace(
        ...
        function (match: string, variable: string) {
          return "'\\" + variable + "'";
        },
      );
    },
