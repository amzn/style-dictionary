 ...
 * });
 * ```
 * @param {TransformedToken} a - first element for comparison
 * @param {TransformedToken} b - second element for comparison
 ...
 */
export default function sortByName(a: TransformedToken, b: TransformedToken) {
  if (b.name > a.name) {
    return -1;
  } else {
    return 1;
