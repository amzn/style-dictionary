...
import { getReferences } from '../../utils/references/getReferences';
import type { TransformedToken, TransformedTokens } from '../../types';
...
export default function sortByReference(
  ...
  opts: { unfilteredTokens?: TransformedTokens },
) {
  /**
   * The sorter function is recursive to account for multiple levels of nesting
   * @param {TransformedToken} a
   * @param {TransformedToken} b
   * @returns
   */
  function sorter(a: TransformedToken, b: TransformedToken) {
    const aComesFirst = -1;
    const bComesFirst = 1;
