...
import type { Dictionary, TransformedTokens, TransformedToken, Filter, Config } from './types';

/**
 * @param {TransformedToken[]} arr
 * @param {Filter['filter']} predicate
 * @param {Config} options
 */
async function asyncFilter(arr: TransformedToken[], predicate: Filter['filter'], options: Config) {
 ...
 * @param {Config} options
 * @returns {Promise<TransformedTokens>} tokens - A new object containing only the tokens
 *   that matched the filter.
 */
async function filterTokenObject(
  ...
  // out
  const result: TransformedTokens = await Object.entries(tokens ?? []).reduce(
    async (_acc, [key, token]) => {
      ...
      if (!isPlainObject(token)) {
        ...
        const filtered = await asyncFilter([token as TransformedToken], filter, options);
 ...
 */
export default async function filterTokens(
