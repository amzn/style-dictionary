...
import type { Expand, ExpandFilter, PlatformConfig } from '../types/Config';
import type { DesignToken, Config, DesignTokens } from '../types';
 ...
 * @returns {string}
 */
export function getTypeFromMap(
  subtype: string,
  compositionType: string,
  expandTypesMap: Expand['typesMap'] = {},
) {
 ...
 */
function shouldExpand(token: DesignToken, opts: Config, platform?: PlatformConfig) {
  const expand = platform?.expand ?? opts.expand ?? false;

  let condition: ExpandFilter | boolean = false;
  let reverse = false;

  if (typeof expand === 'function' || typeof expand === 'boolean') {
    condition = expand;
  } else {
    const type = opts.usesDtcg ? token.$type : token.type;
 ...
 * @param {PlatformConfig} [platform]
 */
export function expandToken(token: DesignToken, opts: Config, platform?: PlatformConfig) {
  ...
  const compositionType = token.$type ?? (token.type as string);
  const expandedTokenObj: DesignTokens = {};
  let typesMap: Expand['typesMap'] = {};
 ...
 * @param {PlatformConfig} [platform]
 */
export function expandTokens(dictionary: DesignTokens, opts: Config, platform?: PlatformConfig) {
