...
import type { DesignTokens, DesignToken } from '../types';

/**
 * @param {DesignTokens} slice
 * @param {{applyTypesToGroup?: boolean}} [opts]
 */
function recurse(slice: DesignTokens, opts: { applyTypesToGroup?: boolean }) {
 ...
 * @param {{applyTypesToGroup?: boolean}} [opts]
 */
export function convertToDTCG(dictionary: DesignTokens, opts: { applyTypesToGroup?: boolean }) {
 ...
 * @param {Entry} entry
 */
export async function resolveZIPEntryData(entry: Entry) {
  let data;
  if (entry.getData) {
 ...
 */
export async function readZIP(zipBlob: Blob) {
 ...
 */
export async function writeZIP(zipEntries: Record<string, string>) {
 ...
 * @param {string} type
 */
async function blobify(blobOrPath: Blob | string, type: string) {
 ...
 * @param {string} [path]
 */
function validateBlobType(blob: Blob, type: string, path?: string) {
 ...
 * @param {{applyTypesToGroup?: boolean}} [opts]
 */
export async function convertJSONToDTCG(
  blobOrPath: Blob | string,
  opts: { applyTypesToGroup?: boolean },
) {
 ...
 * @param {{applyTypesToGroup?: boolean}} [opts]
 */
export async function convertZIPToDTCG(
  blobOrPath: Blob | string,
  opts: { applyTypesToGroup?: boolean },
) {
  ...
  const convertedZipObject = Object.fromEntries(
    Object.entries(zipObjectWithData).map(([fileName, data]) => [
      fileName,
      JSON.stringify(convertToDTCG(JSON.parse(data as string), opts), null, 2),
    ]),
  );
