...
import type { DesignToken, DesignTokens, TransformedToken, TransformedTokens } from '../types';

/**
 * @private
 * @param  {DesignTokens} slice - The plain object you want flattened into an array.
 ...
 * @return {Token[]}
 */
function _flattenTokens<T extends DesignToken | TransformedToken>(
  slice: DesignTokens | TransformedTokens,
  usesDtcg: boolean,
  to_ret: T[] = [],
): T[] {
 ...
 * be more generic.
 * @param  {DesignTokens} tokens - The plain object you want flattened into an array.
 * @param {boolean} [usesDtcg] - Whether or not tokens are using DTCG syntax.
 * @return {Token[]}
 */
export default function flattenTokens<T extends DesignToken | TransformedToken>(
