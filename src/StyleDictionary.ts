...
import GroupMessages, { verbosityInfo } from './utils/groupMessages';
import flattenTokens from './utils/flattenTokens';
import { detectDtcgSyntax } from './utils/detectDtcgSyntax';
import { preprocess } from './utils/preprocess';
import { typeDtcgDelegate } from './utils/typeDtcgDelegate';
import createFormatArgs from './utils/createFormatArgs';
import { deepmerge } from './utils/deepmerge';
import { expandTokens } from './utils/expandObjectTokens';
...
export class StyleDictionary extends Register {
  ...
  public allTokens?: TransformedToken[];
  public log?: LogConfig;
  public source?: string[];
  public include?: string[];
  public expand?: ExpandConfig;
  public platforms?: Record<string, PlatformConfig>;
  public parsers?: string[];
  public preprocessors?: string[];
  public usesDtcg?: boolean;
  public volume?: Volume;
  public unfilteredTokens?: TransformedTokens;
  public unfilteredAllTokens?: TransformedToken[];
  public hasInitialized?: Promise<unknown>;
  public hasInitializedResolve?: (value?: unknown) => void;
   ...
   * @returns
   */
  async init(opts: { verbosity?: LogConfig['verbosity']; warnings?: LogConfig['warnings'] }) {
   ...
   */
  async extend(
    config: Config | string = this.config,
    opts: {
      mutateOriginal?: boolean;
      verbosity?: LogConfig['verbosity'];
      warnings?: LogConfig['warnings'];
    ...
    if (!opts.mutateOriginal) {
      const newSD = new StyleDictionary(deepmerge(this.options, config as Config), {
        init: false,
    ...
    if (typeof config === 'string') {
      ...
      if (['json', 'json5', 'jsonc'].includes(ext)) {
        options = JSON5.parse(this.volume?.readFileSync(cfgFilePath, 'utf-8') as string);
      } else {
    ...
    Object.entries(this.options).forEach(([key, val]) => {
      ...
      const _key = key as keyof StyleDictionary;
      this[_key] = val;
    });
    ...
    if (this.source) {
      ...
      const result = await combineJSON(
        ...
        function Collision(prop: DesignToken) {
          GroupMessages.add(
   ...
   * @returns
   */
  shouldRunExpansion(expandCfg?: ExpandConfig) {
   ...
   */
  async exportPlatform(platform: string): Promise<TransformedTokens> {
    ...
    if (this.shouldRunExpansion(platformConfig.expand)) {
      platformProcessedTokens = expandTokens(
  ...
  async getPlatform(platform: string) {
    ...
    this.allTokens = flattenTokens(tokens, this.usesDtcg);
  ...
  async formatFile(
    ...
    platform: TransformedPlatformConfig,
    dictionary: Dictionary,
  ) {
    await this.hasInitialized;
    const logs: Record<'warning' | 'success', string[]> = {
      warning: [],
      success: [],
    };
    ...
    const filteredDictionary = {
      ...
      unfilteredAllTokens: flattenTokens<TransformedToken>(dictionary.tokens, this.usesDtcg),
    };
     ...
     */
    const nameCollisionObj: Record<string, TransformedToken[]> = {};
   ...
   * @param {string} platform
   */
  async formatPlatform(platform: string) {
    await this.hasInitialized;
  ...
  async formatAllPlatforms() {
    ...
    const getOutputsForPlatform = async (platformKey: string) => {
   ...
   * @returns
   */
  async buildPlatform(platform: string) {
    await this.hasInitialized;
    const { dictionary, platformConfig } = await this.getPlatform(platform);
     ...
     * @param {string} output
     */
    const writeFile = async (destination: string, output: string) => {
